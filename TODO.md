...
| Frontend Unit Tests    | ~85      | 560       | ✅ 659%     |
...
### React Infinite Loop Fix (2026-01-19)

**Issue:** React components re-rendering infinitely due to TanStack Query `onSuccess` callbacks creating new state objects.

**Root Cause:** TanStack Query v5 deprecated `onSuccess` in favor of `useEffect` patterns. Using `onSuccess` to update Zustand store caused infinite loops.

**Fix Applied:**
- `folder-sidebar.tsx`: Changed `useEnhancedChatStore(...)` to `useEnhancedChatStore.getState()` in useQuery callbacks
- `agent-selector.tsx`: Same pattern applied

**Commits:**
| Commit  | Description                                    |
| ------- | ---------------------------------------------- |
| f6cefe1 | fix: Resolve React infinite loop in chat components |
| 77e2e66 | fix: Prevent infinite re-renders in chat components |

### AbortController Test Fix (2026-01-19)

**Issue:** 2 tests failing in `chat-api.test.ts` due to jsdom's AbortController incompatibility with mocked fetch.

**Root Cause:** jsdom's AbortController implementation throws errors when accessed after mocking global.fetch.

**Fix Applied:**
- Added `MockAbortController` class with proper signal property
- Added `afterAll` hook to restore original AbortController

**Commit:**
| Commit  | Description                                    |
| ------- | ---------------------------------------------- |
| 2d832be | fix: Add MockAbortController for jsdom compatibility |
